...
try:
    ...
    _logger.error('Odoo module legion_hr_linkedin_recruitment depends on the several '
...
class LinkedinSocial(http.Controller):
    ...
    @http.route('/linkedin/callback', type='http', website=True, auth='public')
    def social_linkedin_callbacks(self):
        ...
        linkedin_auth_provider = request.env.ref(
            'legion_hr_linkedin_recruitment.provider_linkedin')
        ...
        # Parse and extract the text from the <p> tag in the job description
        html_content = linked_in_url.description
        if html_content:
        ...
        payload = json.dumps({
            ...
            "specificContent": {
                "com.linkedin.ugc.ShareContent": {
                    "shareCommentary": {
                        "text": "\n".join(filter(None, [
                            f"Job Title: {safe_get(linked_in_url.name)}" if linked_in_url.name else None,
                            f"Job Description: {safe_get(p_text)}" if p_text else None,
                            f"Company: {safe_get(linked_in_url.company_id.name)}" if linked_in_url.company_id and linked_in_url.company_id.name else None,
                            f"Department: {safe_get(linked_in_url.department_id.name)}" if linked_in_url.department_id and linked_in_url.department_id.name else None,
                            f"Contract Type: {safe_get(linked_in_url.contract_type_id.name)}" if linked_in_url.contract_type_id and linked_in_url.contract_type_id.name else None,
                            f"Industry: {safe_get(linked_in_url.industry_id.name)}" if linked_in_url.industry_id and linked_in_url.industry_id.name else None,
                            # f"Schedule: {safe_get(linked_in_url.schedule_id.name)}" if linked_in_url.schedule_id and linked_in_url.schedule_id.name else None,
                            "Skills: " + ', '.join([safe_get(skill.name) for skill in
                                                    linked_in_url.skill_ids]) if linked_in_url.skill_ids else None,
                            "Location: " + ", ".join(filter(None, [
                                safe_get(linked_in_url.address_id.street),
                                safe_get(linked_in_url.address_id.city),
                                safe_get(linked_in_url.address_id.state_id.name),
                                safe_get(linked_in_url.address_id.zip),
                                safe_get(linked_in_url.address_id.country_id.name)
                            ])) if any([linked_in_url.address_id.street, linked_in_url.address_id.city,
                                        linked_in_url.address_id.state_id.name, linked_in_url.address_id.zip,
                                        linked_in_url.address_id.country_id.name]) else None,
                            # f"Salary: ${safe_get(linked_in_url.salary_min)} - ${safe_get(linked_in_url.salary_max)}" if linked_in_url.salary_min and linked_in_url.salary_max else None,
                        ]))
                    },
                    ...
                    "media": [
                        {
                            "status": "READY",
                            "description": {
                                "text": linked_in_url.description if linked_in_url.description  else '',
                            },
